...
@pytest.mark.asyncio
async def test_delete_product_successfully(id_generator, temp_product_repository):
  ...
  command = DeleteProductCommand(product_repository= temp_product_repository)
  ...
  deleted_product = await temp_product_repository.find_one(created_product_id)

  assert deleted_product == None

@pytest.mark.asyncio
async def test_delete_failure_user_not_found(id_generator, temp_product_repository):
  command = DeleteProductCommand(product_repository= temp_product_repository)
