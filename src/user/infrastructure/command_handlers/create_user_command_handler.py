...
from common.infrastructure.events.diator.diator_event_publisher import DiatorEventMapper, DiatorEventPublisher
...
from user.infrastructure.events.user_created.client_created import client_created_diator_adapter
from user.infrastructure.events.user_created.manager_created import manager_created_diator_adapter
...
class CreateUserCommandHandler(RequestHandler[CreateUserCommand, CreateUserResponse]):
    ...
    def __init__(self,
        ...
        self._event_publisher = DiatorEventPublisher([
            DiatorEventMapper(event=ClientCreated,adapter=client_created_diator_adapter),
            DiatorEventMapper(event=ManagerCreated,adapter=manager_created_diator_adapter)
        ])
