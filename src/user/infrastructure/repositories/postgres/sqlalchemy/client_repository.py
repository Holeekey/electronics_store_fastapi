...
class ClientRepositorySqlAlchemy(IClientRepository):
    ...
    async def find_one(self, id: ClientId):
        user_orm = self.db.query(UserModel).filter(UserModel.id == id.id.__str__()).filter(UserModel.role == UserRole.CLIENT.name).first()
        if is_none(user_orm):
            return None
    ...
    async def find_all(self):
        users_orm = self.db.query(UserModel).filter(UserModel == UserRole.CLIENT.name).all()
        return [client_factory(
            id=user_orm.id,
            first_name=user_orm.first_name,
            last_name=user_orm.last_name,
