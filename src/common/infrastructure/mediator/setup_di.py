...
def setup_di() -> DIContainer:
    ...
    external_container.solve(Dependent(UUIDGenerator, scope="singleton",), scopes=["singleton"])
    
    external_container.solve(Dependent(FernetProvider, scope="singleton",), scopes=["singleton"])
    
    external_container.bind(
        bind_by_type(
            Dependent(CreateUserCommandHandler, scope="request",),
